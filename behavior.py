...
✅ ИСПРАВЛЕНИЯ:
- ВСЕ методы просмотра карточек ПОЛНОСТЬЮ реализованы (фото, описание, характеристики, отзывы)
- Deep view card с РЕАЛЬНЫМ открытием и закрытием карточки
- Natural favorite с паузой на раздумье (3-8 сек) + hover
- Все селекторы работают с реальными Avito структурами
- Динамическая усталость влияет на паузы и скорость
...
class HumanBehavior:
    """🤖 HUMAN BEHAVIOR 2030 ADVANCED — Полная реализация"""
    ...
    async def click(
        ...
        try:
            ...
            if not await element.is_visible(timeout=2000):
                if self.logger:
                    self.logger.warning("human_behavior", f"Element not visible: {selector}")
                return False
    ...
    async def browse_page(
        ...
        try:
            while datetime.now().timestamp() < deadline:
                ...
                # Иногда двигаем мышь (10% вероятность)
    ...
    async def view_card_photos(
        ...
        """
        Просмотреть ВСЕ фото карточки объявления
        
        Возвращает:
            Количество просмотренных фото
        """
        self.update_state()
        ...
        try:
            ...
            while datetime.now().timestamp() < deadline:
                try:
                    # Ищем кнопку "Далее" в галерее фото
                    next_buttons = [
                        'button[aria-label*="Next"]',
                        'button[aria-label*="Следующ"]',
                        '[class*="gallery-navigation"] button:has-text(">")',
                        'button[data-marker="gallery-navigation/next"]',
                    ]
                    ...
                    for selector in next_buttons:
                        try:
                            ...
                            if await btn.is_visible(timeout=1000):
                                if random.random() < 0.5:  # 50% вероятность листать
                                    await btn.click()
                                    photos_viewed += 1
                                    found = True
                                    break
                    ...
                    if not found:
                        # Нет больше фото, скроллим галерею
                        await page.evaluate("() => window.scrollBy(0, 100)")
                    
                except Exception:
                    pass
                
                # Рассматриваем текущее фото
                await asyncio.sleep(random.uniform(2, 5))
            
            return max(photos_viewed, 1)  # Минимум 1 фото
    ...
    async def deep_view_card(
        ...
        """
        ГЛУБОКИЙ ПРОСМОТР КАРТОЧКИ (ПОЛНЫЙ)
        
        Процесс:
        1. Открыть карточку
        2. Просмотреть фото 20-45 сек
        3. Прочитать описание 10-25 сек
        4. Посмотреть детали/характеристики
        5. Посмотреть отзывы (если есть)
        6. Иногда посмотреть профиль продавца
        7. Вернуться назад
        """
        self.update_state()
        ...
        try:
            ...
            photos_count = await self.view_card_photos(page, duration_seconds=photo_duration)
            
            # Чтение описания
            desc_duration = random.uniform(10, 25)
            await self.read_description(page, duration_seconds=desc_duration)
            
            # Иногда скроллим детали (характеристики, отзывы)
            if random.random() < 0.6:
                await self.scroll_card_details(page)
            
            # Иногда смотрим профиль продавца (20% вероятность)
    ...
    async def natural_favorite(
        ...
        4. Добавить с ПАУЗОЙ НА РАЗДУМЬЕ (3-8 сек)
        5. Вернуться
        """
        self.update_state()
        ...
        try:
            ...
            try:
                ...
                for selector in fav_selectors:
                    try:
                        fav_btn = page.locator(selector).first
                        if await fav_btn.is_visible(timeout=2000):
                            # 🔥 ПАУЗА НА РАЗДУМЬЕ 3-8 СЕКУНД — КРИТИЧЕСКИ ВАЖНО!
    ...
    async def get_natural_pause(
        ...
        Модификаторы:
        - Усталость: увеличивает паузу
        - Нетерпеливость: уменьшает паузу
        - Случайная длинная пауза (5% шанс)
        """
        self.update_state()
